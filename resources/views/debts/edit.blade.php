<x-app-layout>
    <x-slot name="header">
        ...
        <p class="text-sm text-slate-500 mt-1">Ubah rincian informasi hutang untuk pelanggan {{ $debt->customer->name ?? '—' }}.</p>
    </x-slot>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <select id="customer_id" name="customer_id" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-sm" required>
                                    ...
                                    @foreach ($customers as $customer)
                                        <option value="{{ $customer->id }}" {{ old('customer_id', $debt->customer_id) == $customer->id ? 'selected' : '' }}>
                                            {{ $customer->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="status" name="status" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-sm" required>
                                    <option value="unpaid" {{ old('status', $debt->status) == 'unpaid' ? 'selected' : '' }}>Belum Lunas</option>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <input id="total_amount" name="total_amount" type="number" step="0.01" value="{{ old('total_amount', $debt->total_amount) }}" placeholder="0" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-sm" required>
                            </div>
                            ...
                            <div>
                                ...
                                <input id="remaining_amount" name="remaining_amount" type="number" step="0.01" value="{{ old('remaining_amount', $debt->remaining_amount) }}" placeholder="0" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-sm" required>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <input id="debt_date" name="debt_date" type="date" value="{{ old('debt_date', $debt->debt_date) }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-sm" required>
                            </div>
                            
                            <div>
                                <label class="block text-sm font-bold text-slate-700 mb-2" for="due_date">Tanggal Jatuh Tempo</label>
                                <input id="due_date" name="due_date" type="date" value="{{ old('due_date', $debt->due_date) }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-sm">
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2" for="notes">Catatan Transaksi</label>
                            <textarea id="notes" name="notes" rows="3" placeholder="Contoh: Pembelian karung goni bekas 100 lembar" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-sm">{{ old('notes', $debt->notes) }}</textarea>
                        </div>
