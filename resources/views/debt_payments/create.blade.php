<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-2xl text-slate-800 leading-tight">
            {{ __('Catat Pembayaran Hutang') }}
        </h2>
        <p class="text-sm text-slate-500 mt-1">Catat transaksi pembayaran cicilan atau pelunasan hutang pelanggan.</p>
    </x-slot>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <select id="debt_id" name="debt_id" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-sm" required>
                                ...
                                @foreach ($debts as $debt)
                                    <option value="{{ $debt->id }}" {{ old('debt_id') == $debt->id ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <input id="amount_paid" name="amount_paid" type="number" step="0.01" value="{{ old('amount_paid') }}" placeholder="0" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-sm" required>
                            </div>
                            ...
                            <div>
                                ...
                                <input id="payment_date" name="payment_date" type="date" value="{{ old('payment_date', date('Y-m-d')) }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-sm" required>
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2" for="notes">Catatan Pembayaran</label>
                            <textarea id="notes" name="notes" rows="3" placeholder="Contoh: Pembayaran cicilan cash / transfer bank" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-sm">{{ old('notes') }}</textarea>
                        </div>
